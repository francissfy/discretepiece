...
#define CHECK_OR_RETURN(condition)                           \
  if (condition) {                                           \
    ...
    return ::discretepiece::util::StatusBuilder(             \
               ::discretepiece::util::StatusCode::kInternal) \
...
// hash function for vector<char32>
...
template <typename T>
std::vector<std::vector<T>> VectorSplit(const std::vector<T> &src, const T &deliminator) {
