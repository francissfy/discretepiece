...
#define LOG(severity)                                                        \
  (::discretepiece::logging::GetMinLogLevel() >                              \
   ::discretepiece::logging::LOG_##severity)                                 \
      ? 0                                                                    \
      : ::discretepiece::error::Die(                                         \
            ::discretepiece::logging::LOG_##severity >=                      \
            ::discretepiece::logging::LOG_FATAL) &                           \
            std::cerr << ::discretepiece::logging::BaseName(__FILE__) << "(" \
                      << __LINE__ << ") "                                    \
                      << "LOG(" << #severity << ") "

#define CHECK(condition)                                                      \
  (condition) ? 0                                                             \
              : ::discretepiece::error::Die(true) &                           \
                    std::cerr << ::discretepiece::logging::BaseName(__FILE__) \
                              << "(" << __LINE__ << ") [" << #condition       \
                              << "] "
